...
class EventSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $organiserIds = DB::table('users')->where('role', 'organiser')->pluck('id')->toArray();

        if (empty($organiserIds)) {
            $this->command->warn('No organiser users found. Skipping EventSeeder.');
            return;
        }
        ...
        for ($i = 0; $i < 15; $i++) { // Create 15 events
            ...
            DB::table('events')->insert([
                ...
                'user_id' => $faker->randomElement($organiserIds), // Assign a random organizer
