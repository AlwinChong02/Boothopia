<head>
  ...
  <style>
    .booth-unavailable { opacity: .5; pointer-events: none; }
    .booth-card.selected {
  ...
  <div class="container mt-5">
    ...
    @if(Auth::id() === $event->user_id)
      @if($daysUntilStart >= 7 && $event->status !== 'canceled')
        <form
          action="{{ route('events.cancel', ['event' => $event->id]) }}"
          method="POST"
    ...
    {{-- booth booking form --}}
    <form id="boothForm"
          action="{{ route('events.booking.process', ['event' => $event->id]) }}"
          method="POST"
    >
      @csrf
              ...
              <div class="card h-100 booth-card {{ $isAvailable ? '' : 'booth-unavailable' }}">
                <div class="card-body">
                  ...
                  <div class="form-check">
                    <input class="form-check-input booth-checkbox"
                           type="checkbox"
                           name="booths[]"
                           value="{{ $booth->id }}"
                           id="booth{{ $booth->id }}"
