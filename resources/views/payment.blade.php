    ...
    <div class="container mt-5">
        ...
        <form method="POST" action="{{ route('payment.approval') }}" enctype="multipart/form-data" class="mt-4">
            ...
            <div class="mb-3">
                ...
                {{-- hidden value for event_id organiser id(users role is organiser), requester id, booth_quantity --}}
                <input type="hidden" name="event_id" value="{{ $event->id }}">
                <input type="hidden" name="organiser_id" value="{{ $event->user_id }}">
                <input type="hidden" name="requester_id" value="{{ auth()->user()->id }}">
                <input type="hidden" name="booth_quantity" value="{{ count($booths) }}">
                <input type="hidden" name="booth_ids"
                    value="{{  implode(',', $booths->pluck('id')->toArray()) }}">
                {{-- <div class="alert alert-info mt-2">
                    <strong>Debug booth_ids:</strong> {{ implode(',', $booths->pluck('id')->toArray()) }}
                </div> --}}
